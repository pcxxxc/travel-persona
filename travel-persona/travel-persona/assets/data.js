...
function inferPersona(userProfile) {
  ...
  // 基于节奏偏好
  if (userProfile.pacePref >= 4) {
    scores.relax_roamer = (scores.relax_roamer || 0) + 3;
    scores.nature_healer = (scores.nature_healer || 0) + 2;
    scores.nomad_trial = (scores.nomad_trial || 0) + 2;
  } else if (userProfile.pacePref <= 2) {
    scores.efficient_checker = (scores.efficient_checker || 0) + 3;
    scores.street_explorer = (scores.street_explorer || 0) + 1;
  }

  // 基于空间偏好
  if (userProfile.spacePrefs) {
    for (var i = 0; i < userProfile.spacePrefs.length; i++) {
      ...
      if (sp === '小镇慢生活') { scores.relax_roamer = (scores.relax_roamer || 0) + 1; scores.nature_healer = (scores.nature_healer || 0) + 1; }
    }
  }
  ...
  if (userProfile.emotionGoal) {
    var emotionMap = {
      '放空': ['relax_roamer', 'nature_healer'],
      '逃离压力': ['nature_healer'],
      '找灵感': ['creative_collector'],
      '拍照出片': ['creative_collector', 'relax_roamer'],
      '社交': ['street_explorer'],
      '独处整理': ['relax_roamer', 'nature_healer'],
      '试住城市': ['nomad_trial']
    };
    var mapped = emotionMap[userProfile.emotionGoal] || [];
    for (var j = 0; j < mapped.length; j++) {
      scores[mapped[j]] = (scores[mapped[j]] || 0) + 3;
    }
  }
